...
save to mappings/filenames.csv
...
"""
from openelex.base.datasource import DataSource

class MDData(DataSource):
    ...
    #TODO: datefilter handler; if only years are available, need to either 
    # 1) introspect data for dates or
    # 2) notify that only year-based filter is available (and filter for year?)

    @property
    def target_years(self):
        return range(2000, 2014)
